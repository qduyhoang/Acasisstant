...
metadata, idx_q, idx_a = word_embed.load_data(PATH='')                   

(trainX, trainY), (testX, testY), (validX, validY) = word_embed.split_dataset(idx_q, idx_a)
...
for epoch in range(n_epoch):
    ...
    for X, Y in tl.iterate.minibatches(inputs=trainX, targets=trainY, batch_size=batch_size, shuffle=False):
        ...
        ###============= inference
        if n_iter % 1000 == 0:
            seeds = ["philadelphia is a big city",
            ...
            for seed in seeds:
                ...
                for _ in range(5):  # 1 Query --> 5 Reply
                    ...
                    sentence = [w]
                    for _ in range(50): # max sentence length
                        o, state = sess.run([y, net_rnn.final_state_decode],
                                        {net_rnn.initial_state_decode: state,
                                        decode_seqs2: [[w_id]]})
